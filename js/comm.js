var Marke = {
	...
	init: function() {
		//初始化swiper
		var g_Swiper = new Swiper('.g-swiper', {
			loop: false, //可选选项，开启循环
			mode: 'vertical', //纵向滚动
			initialSlide: 0,
			mousewheelControl: true, //开启滚轮
		...
		var organization_Swiper = new Swiper('.organization-swiper', {
			...
			slidesPerView: 1,
			onSlideChangeStart: function(swiper) {
				$(".o-nav li").eq(swiper.activeIndex).addClass("on").siblings().removeClass("on");
			}
...
//			loop: true
		...
		$(".p-btn-r").click(function() {
			partners_Swiper.swipeNext();
		})
		$(".p-btn-l").click(function() {
			partners_Swiper.swipePrev();
		})
		$(".o-btn-r").click(function() {
			organization_Swiper.swipeNext();
		})
		$(".o-btn-l").click(function() {
			organization_Swiper.swipePrev();
		})
		$(".n-btn-r").click(function() {
			news_Swiper.swipeNext();
		})
		$(".n-btn-l").click(function() {
			news_Swiper.swipePrev();
		})
		//首页切换
//		$(".btn-about").click(function() {
//			$(this).hide().siblings(".content-list").hide().siblings(".content-info-list").fadeIn();;
//		})
		//跳到某个swiper页
		Marke.swiperTo(".nav-list li", g_Swiper);
		Marke.swiperTo(".o-nav li", organization_Swiper);
	...
	swiperTo: function(class_name, swiper_name) {
		$(class_name).click(function() {
			...
			swiper_name.swipeTo(index, 500, false)
			$(".m-container").removeClass("animaiton");
